...
final class HomeIdpDiscoverer {
  ...
  public List<IdentityProviderModel> discoverForUser(String username) {
    ...
    LOG.tracef(
      ...
      username,
      realmName,
      authenticatorConfig == null ? "<unconfigured>" : authenticatorConfig.getAlias()
    );
  ...
  private List<IdentityProviderModel> discoverHomeIdps(Domain domain, UserModel user, String username) {
    ...
    if (user == null || !config.forwardToLinkedIdp()) {
      ...
      LOG.tracef("Found local user '%s' and forwarding to linked IdP is enabled. Discovering linked IdPs.", username);
      linkedIdps =
        context
          .getSession()
          .users()
    ...
    List<IdentityProviderModel> enabledIdpsWithMatchingDomain = filterIdpsWithMatchingDomainFrom(
      enabledIdps,
      domain,
      config
    );
  ...
  private List<IdentityProviderModel> getLinkedIdpsFrom(
    ...
    return enabledIdpsWithMatchingDomain
      .stream()
      .filter(it -> linkedIdps.containsKey(it.getAlias()))
      .collect(Collectors.toList());
  }

  private List<IdentityProviderModel> filterIdpsWithMatchingDomainFrom(
    List<IdentityProviderModel> enabledIdps,
    Domain domain,
    HomeIdpDiscoveryConfig config
  ) {
    String userAttributeName = config.userAttribute();
    List<IdentityProviderModel> idpsWithMatchingDomain = enabledIdps
      .stream()
  ...
  private List<IdentityProviderModel> determineEnabledIdps() {
    ...
    List<IdentityProviderModel> enabledIdps = realm
